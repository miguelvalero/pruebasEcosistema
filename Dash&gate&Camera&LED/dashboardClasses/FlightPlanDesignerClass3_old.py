...
class FlightPlanDesignerWindow:
    ...
    def openWindowToCreateFlightPlan(self):
            ...
            self.clearButton = tk.Button(self.newWindow, width = 50, height = 3, text="Clear",  bg='red', fg="white", command=self.clear)
            self.clearButton.grid(row=2, column=0, rowspan = 2)

            self.sliderFrame= tk.LabelFrame(self.newWindow, text = 'Select distance in meters for spiral', width = 100, height = 3)
    ...
    def click(self, e):
            if self.done:
                ...
                self.sliderFrame = tk.LabelFrame(self.newWindow, width=60,
                                                 text='Select distance in meters for spiral')
                self.sliderFrame.grid(row=2, column=1, padx=0, pady=0)
